...
import type { ProjectTask } from "./project-kanban-board";
import { deleteTaskAction, updateTaskAction } from "@/app/(dashboard)/tasks/actions";

interface ProjectKanbanTaskProps {
  task: ProjectTask;
  isDragging?: boolean;
}

export function ProjectKanbanTask({ task, isDragging = false }: ProjectKanbanTaskProps) {
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={`group rounded-lg border border-white/10 bg-white/5 p-4 transition-all hover:border-white/20 hover:bg-white/10 shadow ${
    ...
    >
      <div className="mb-3 flex items-start justify-between gap-2">
        <h4
          {...attributes}
          {...listeners}
          className="flex-1 cursor-grab text-base font-semibold text-white leading-snug active:cursor-grabbing"
        >
          {task.title}
        </h4>
      ...
      {task.description && (
        <p className="mb-3 text-sm text-slate-300 leading-relaxed">{task.description}</p>
      )}
      ...
      {isOverdue && (
        <div className="mt-2 flex items-center gap-1.5 text-sm text-red-400 font-medium">
          <span>⚠️</span>
